...
export function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-4 border-b border-white/5 bg-ink-900/70 px-4 backdrop-blur-md sm:px-6">
      {/* Org context */}
      <div className="hidden items-center gap-2 md:flex">
        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-ink-600 text-sm font-bold text-cyber">
          {org.name.charAt(0)}
        </div>
      ...
      <div className="relative ml-auto hidden max-w-sm flex-1 sm:block">
        ...
        <input
          ...
          className="h-9 w-full rounded-lg border border-white/10 bg-ink-700/60 pl-9 pr-3 text-sm text-slate-200 placeholder:text-slate-500 ring-focus"
        />
      </div>
      ...
      <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-ink-700/60 p-0.5">
        {ROLES.map((r) => (
          <button
            key={r}
            onClick={() => setRole(r)}
